...
class GetWidgets extends Action
{
   ...
   protected function onGetWidgets()
   {
      ...
      $device_type = $this->request->query('device_type', Widget::DEVICE_TYPES['all']);
      ...
      if (isset($device_type) && $device_type != Widget::DEVICE_TYPES['all']) {
         $query = $query->where(function ($query) use ($device_type) {
            $query->where('device_type', $device_type)
               ->orWhere('device_type', Widget::DEVICE_TYPES['all']);
         });
      }
